...
当初はクライアント実装をスコープ外としていた。プロダクトごとに選択が分かれると見ていたためである。実際には配布先(iOS / Android)もバックエンド(Workers + D1)も共通であり、各プロダクトが同じ判断を独立に繰り返すだけになっていた。標準構成を1つ持ち、合わないプロダクトがそこだけ外すほうが総量が小さい。ストア配布の運用だけは引き続き引き受けない。証明書と審査はアカウントに固有であり、共通化できる部分が無い。
...
* GitHub Actions の消費はこのランニングコストとは別枠として扱う。public リポジトリでは標準 runner が無料である。private リポジトリでは macOS が10倍係数で included minutes を消費し、Free 枠の 2,000分 は macOS 約200分に相当する。iOS ビルドを CI で回すプロダクトは、リポジトリを public にするか、iOS ビルドの実行可否を明示的に選ぶ。可視性はプロダクトごとの判断とし、プラットフォームは既定を「実行しない」に置く。
...
この表は Node ランタイムのプロダクトを対象とする。Flutter クライアント(モノレポの `app/`)は `flutter` コマンドを正とし、薄い npm スクリプトで包み直さない。包んでも実体は `flutter` であり、規約を満たすためだけの層が1枚増えるだけである。コマンド名を揃える代わりに、CI 側を領域ごとの reusable workflow に分けて差を吸収する(`flutter-ci.yml`)。モノレポでは `paths` で変更を検知し、`app/` と `worker/` の job を分ける。
